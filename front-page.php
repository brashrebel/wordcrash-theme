...
?>
	...
	<?php if ( !empty( $user_query ) ) { ?>
			<ul class="crash-pad-list small-block-grid-4 user-grid">
				...
				<li>
					<div class="crash-pad">
						...
						<p><strong>Pets:</strong> <?php echo get_user_meta( $user->ID, 'pets', true ); ?><br/>
							<strong>Capacity:</strong> <?php echo get_user_meta( $user->ID, 'capacity', true ); ?></p>
						<a href="#" class="modal-trigger" data-user-id="<?php echo $user->ID; ?>" data-reveal-id="gf-ping-form"><i class="fa fa-thumbs-up"></i></a>
					</div>
				</li>
		<?php } ?>
			</ul>
